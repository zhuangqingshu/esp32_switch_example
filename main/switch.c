...
#define LIGHT_MESH_INIT_CONFIG_STORE_KEY "init_config"
#define LIGHT_MESH_AP_CONFIG_STORE_KEY   "ap_config"

#define HLW8032_UART_NUM UART_NUM_1
#define HLW8032_UART_TX_IO_NUM GPIO_NUM_19
#define HLW8032_UART_RX_IO_NUM GPIO_NUM_22
...
void node_handle_task(void *arg)
{
    ...
    while (true) {
        ...
        MDF_LOGD("Node receive, addr: " MACSTR ", size: %d, data: %.*s", MAC2STR(src_addr), size, size, data);
...
static void light_led_show_key_release()
{
	if (!mwifi_is_connected()) {
		light_driver_led_blink_start(0, 0, 255, 2000);
	} else {
		light_driver_led_set_rgb(0, 0, 255);
	}
}
...
static mdf_err_t event_loop_cb(mdf_event_loop_t event, void *ctx)
{
    ...
    switch (event) {
        ...
        case MDF_EVENT_MWIFI_PARENT_CONNECTED:
            ...
            mdf_info_load(LIGHT_MESH_AP_CONFIG_STORE_KEY, &ap_config, sizeof(mwifi_config_t));
            ...
            if (ap_config.mesh_type != mesh_type) {
                ...
                mdf_info_save(LIGHT_MESH_AP_CONFIG_STORE_KEY, &ap_config, sizeof(mwifi_config_t));
            }
        ...
        case MDF_EVENT_MUPGRADE_STARTED:
			...
			light_driver_led_blink_start(0, 0, 255, 100); /**< blue blink */
...
void app_main()
{
...
#if (defined PM_HLW8032)
	DriverPM_Init(HLW8032_UART_NUM,HLW8032_UART_TX_IO_NUM, HLW8032_UART_RX_IO_NUM);
#endif
     ...
     */
	ret = mdf_info_load(LIGHT_MESH_INIT_CONFIG_STORE_KEY, &init_config, sizeof(mwifi_init_config_t));
	ret |= mdf_info_load(LIGHT_MESH_AP_CONFIG_STORE_KEY, &ap_config, sizeof(mwifi_config_t));
	if ( ret != MDF_OK ) {
		MDF_ERROR_ASSERT(get_network_config(&init_config, &ap_config, DEVICE_TID, DEVICE_NAME));
         ...
         */
		mdf_info_save(LIGHT_MESH_INIT_CONFIG_STORE_KEY, &init_config, sizeof(mwifi_init_config_t));
		mdf_info_save(LIGHT_MESH_AP_CONFIG_STORE_KEY, &ap_config, sizeof(mwifi_config_t));
    ...
    MDF_ERROR_ASSERT(mlink_add_device(DEVICE_TID, name, CONFIG_LIGHT_VERSION));
    MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_SWITCH_0, "onoff_0", CHARACTERISTIC_FORMAT_INT, CHARACTERISTIC_PERMS_RWT, 0, 2, 1));
#if (defined CONFIG_SWITCH_CHANNEL_4) || (defined CONFIG_SWITCH_CHANNEL_3) || (defined CONFIG_SWITCH_CHANNEL_2)
    MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_SWITCH_1, "onoff_1", CHARACTERISTIC_FORMAT_INT, CHARACTERISTIC_PERMS_RWT, 0, 2, 1));
#endif
#if (defined CONFIG_SWITCH_CHANNEL_4) || (defined CONFIG_SWITCH_CHANNEL_3)
	MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_SWITCH_2, "onoff_2", CHARACTERISTIC_FORMAT_INT, CHARACTERISTIC_PERMS_RWT, 0, 2, 1));
#endif
#if (defined CONFIG_SWITCH_CHANNEL_4)
	MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_SWITCH_3, "onoff_3", CHARACTERISTIC_FORMAT_INT, CHARACTERISTIC_PERMS_RWT, 0, 2, 1));
#endif
    MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_MODE, "mode", CHARACTERISTIC_FORMAT_INT, CHARACTERISTIC_PERMS_RW, 1, 2, 1));
	MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_POWER, "power", CHARACTERISTIC_FORMAT_DOUBLE, CHARACTERISTIC_PERMS_READ, 1, 1000000, 1));
	MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_CURRENT, "current", CHARACTERISTIC_FORMAT_DOUBLE, CHARACTERISTIC_PERMS_READ, 1, 1000000, 1));
	MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_VOLTAGE, "voltage", CHARACTERISTIC_FORMAT_DOUBLE, CHARACTERISTIC_PERMS_READ, 1, 1000000, 1));
	MDF_ERROR_ASSERT(mlink_add_characteristic(DEVICE_CID_ENERGY, "energy", CHARACTERISTIC_FORMAT_DOUBLE, CHARACTERISTIC_PERMS_RW, 1, 1000000000, 1));
     ...
     */
    const uint8_t default_group_id[6] = {LIGHT_GROUP_ID};
