...
int restart_count_get()
{
	...
	MDF_LOGI("reset reason: %d", reset_reason);
    ...
    if (reset_reason == POWERON_RESET || reset_reason == RTCWDT_RTC_RESET || reset_reason == SW_CPU_RESET) {
        restart_count++;
 ...
 *     - NULL
 */
void mesh_config_set_factory(mwifi_init_config_t *_init_config, mwifi_config_t *_ap_config)
{
	strcpy(_ap_config->router_ssid, "esp32_mesh_network");
	strcpy(_ap_config->router_password, "88888888");
	
	memset(_ap_config->mesh_id, 0x00, sizeof(_ap_config->mesh_id));
	_ap_config->mesh_id[0] = 0x80;
	_ap_config->mesh_id[5] = 0x01;
	strcpy(_ap_config->mesh_password, "88888888");
	_ap_config->channel = 0;
	_ap_config->channel_switch_disable = false;
	_ap_config->router_switch_disable = false;
}
